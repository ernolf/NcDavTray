...
$State = [pscustomobject]@{
	...
	Label     = ''
	IntervalS = 15
	EncPass   = ''	# Installed: DPAPI; Portable: unused
	LangPref  = ''	# '' => auto/system, or 'en','de',...
}
$script:PlainPassCache   = $null
$script:AskedPassphrase  = $false
$script:NeedsSetup       = $true
$script:mapMutexObj      = $null
$script:mapMutexName     = $null
$script:mapMutexOwned    = $false
$script:Paused           = $false
...
function New-StateConfigObject {
	$ht = @{}
	foreach ($k in $script:ConfigKeys) { $val = $State.$k; if ($k -eq 'IntervalS') { $val = [int]$val }; $ht[$k] = $val }
	return New-Object psobject -Property $ht
}
# Build export object for Export-InstalledConfig (adds DPAPI secret as 'DPAPI')
function New-InstalledExportObject {
	$obj = New-StateConfigObject; $dp = $State.EncPass
	Add-Member -InputObject $obj -MemberType NoteProperty -Name 'DPAPI' -Value $dp -Force
	return $obj
}
...
# ================================================================
function Hook-FormDpi([System.Windows.Forms.Form]$f) {
	if (-not $f) { return }
	$f.AutoScaleMode = 'Dpi'
}
...
function Get-ThisScriptPath {
	[CmdletBinding()]
	param([string]$FallbackName = "$($AppNameShort).ps1")
	try {
...
$script:I18N_Embedded_En = @'
{
	...
	"tip.enter_password": "Enter App-Password",
	"tip.enter_password_and_encrypt": "Enter App-Password and click 'Encrypt' to enable folder browse",
	"tip.enter_server": "Enter server address without https://",
	"tip.enter_user": "Enter user login name",
...
function T([string]$Key,[hashtable]$Vars){
...
function Show-CustomMsgBoxT([string]$Key, [hashtable]$Vars = $null, [string]$Mode = 'OK', [string]$Icon = 'None', [string]$Caption = $AppName) {
	...
	switch ($Icon) {
		'Information' { [System.Media.SystemSounds]::Asterisk.Play() }
	...
	switch ($Mode) {
		'OK' {
			$btnDefs = @( @{ Text = T 'button.ok'; Result = [System.Windows.Forms.DialogResult]::OK; Default = $true; Cancel = $true } )
		}
		# FlowDirection = RightToLeft, so first in list ends up rightmost. We add No first, then Yes -> visually "Yes   No"
		'YesNo' {
			$btnDefs = @(
				@{ Text = T 'button.no'; Result = [System.Windows.Forms.DialogResult]::No; Default = $false; Cancel = $true },
				@{ Text = T 'button.yes'; Result = [System.Windows.Forms.DialogResult]::Yes; Default = $true; Cancel = $false }
			)
		}
		# Will render "Yes   No   Cancel"
		'YesNoCancel' {
			$btnDefs = @(
				@{ Text = T 'button.cancel'; Result = [System.Windows.Forms.DialogResult]::Cancel; Default = $false; Cancel = $true },
				@{ Text = T 'button.no'; Result = [System.Windows.Forms.DialogResult]::No; Default = $false; Cancel = $false },
				@{ Text = T 'button.yes'; Result = [System.Windows.Forms.DialogResult]::Yes; Default = $true; Cancel = $false }
			)
		}
		default {
	...
	$icoObj = $null
	switch ($Icon) {
		'Information' { $icoObj = [System.Drawing.SystemIcons]::Information }
		'Warning'     { $icoObj = [System.Drawing.SystemIcons]::Warning }
		'Error'       { $icoObj = [System.Drawing.SystemIcons]::Error }
		'Question'    { $icoObj = [System.Drawing.SystemIcons]::Question }
		default       { $icoObj = $null }
	}
	# ---- Form base ----
	...
	if ($icoObj) {
		$pic.Image = $icoObj.ToBitmap()
	} else {
		# no icon -> collapse spacing
		$pic.Width  = 1
		$pic.Height = 1
		$pic.Margin = New-Object System.Windows.Forms.Padding(0,0,0,0)
	}
	$lbl = New-Object System.Windows.Forms.Label
	$lbl.AutoSize      = $true
	$lbl.MaximumSize   = New-Object System.Drawing.Size(360,0) # wrap text at ~360px
	$lbl.Text          = $bodyText
	[void]$contentPanel.Controls.Add($pic,0,0)
	[void]$contentPanel.Controls.Add($lbl,1,0)
	...
	foreach ($bdef in $btnDefs) {
		...
		$btn.Text         = $bdef.Text
		$btn.DialogResult = $bdef.Result
		if ($bdef.Default) { $defaultBtn = $btn }
		if ($bdef.Cancel)  { $cancelBtn  = $btn }
		[void]$buttonPanel.Controls.Add($btn)
	}
	if ($defaultBtn) { $frm.AcceptButton = $defaultBtn }
	if ($cancelBtn)  { $frm.CancelButton = $cancelBtn }
	# Compose final layout
	[void]$mainPanel.Controls.Add($contentPanel,0,0)
	[void]$mainPanel.Controls.Add($buttonPanel,0,1)
	[void]$frm.Controls.Add($mainPanel)
	# Modal show and return DialogResult
	$dlgResult = $frm.ShowDialog()
	if (-not $dlgResult -or $dlgResult -eq [System.Windows.Forms.DialogResult]::None) {
		if ($cancelBtn) {
			$dlgResult = $cancelBtn.DialogResult
		} else {
			$dlgResult = [System.Windows.Forms.DialogResult]::None
		}
	}
	return $dlgResult
}
# Simple OK/Information message using a localized string
function Show-InfoT([string]$Key,[hashtable]$Vars=$null) { [void](Show-CustomMsgBoxT -Key $Key -Vars $Vars -Mode 'OK' -Icon 'Information' -Caption $AppName) }
# Simple OK/Warning message using a localized string
function Show-WarnT([string]$Key,[hashtable]$Vars=$null) { [void](Show-CustomMsgBoxT -Key $Key -Vars $Vars -Mode 'OK' -Icon 'Warning' -Caption $AppName) }
# Simple OK/Error message using a localized string
function Show-ErrorT([string]$Key,[hashtable]$Vars=$null){ [void](Show-CustomMsgBoxT -Key $Key -Vars $Vars -Mode 'OK' -Icon 'Error' -Caption $AppName) }
# Yes/No question (Information icon); returns DialogResult
function Ask-YesNoInfoT([string]$Key,[hashtable]$Vars=$null){ return Show-CustomMsgBoxT -Key $Key -Vars $Vars -Mode 'YesNo' -Icon 'Information' -Caption $AppName }
# Yes/No question (Warning icon); returns DialogResult
function Ask-YesNoWarnT([string]$Key,[hashtable]$Vars=$null){ return Show-CustomMsgBoxT -Key $Key -Vars $Vars -Mode 'YesNo' -Icon 'Warning' -Caption $AppName }
# Yes/No question (Question icon); returns DialogResult
function Ask-YesNoQuestT([string]$Key,[hashtable]$Vars=$null){ return Show-CustomMsgBoxT -Key $Key -Vars $Vars -Mode 'YesNo' -Icon 'Question' -Caption $AppName }
# Yes/No/Cancel question (Question icon); returns DialogResult
function Ask-YesNoCancelQuestT([string]$Key,[hashtable]$Vars=$null){ return Show-CustomMsgBoxT -Key $Key -Vars $Vars -Mode 'YesNoCancel' -Icon 'Question' -Caption $AppName }
# i18n: custom help dialog with optional clickable URL
function Show-HelpT([string]$TitleKey,[hashtable]$TitleVars=$null,[string]$BodyKey,[hashtable]$BodyVars=$null,[string]$Url=$null,[int]$Width=640,[int]$Height=320) {
	...
	if ($Url) {
		...
		$btnCopy.Margin = New-Object System.Windows.Forms.Padding(8,0,0,0)
...
Set-Item -Path function:\global:Show-HelpT -Value ${function:Show-HelpT} -Force
Write-Verbose ("Has Show-HelpT? {0}" -f [bool](Get-Command Show-HelpT -ErrorAction SilentlyContinue))
# ================================================================
...
function Load-Config {
	if ($PortableMode) {
		if (Test-Path $PortJson) { try { $p = Get-Content -Raw -Path $PortJson | ConvertFrom-Json; Copy-ObjectToState -Source $p } catch {} } else { Save-Config }
	} else {
		if (Test-Path $RegBase) { $p = Get-ItemProperty -Path $RegBase; Copy-ObjectToState -Source $p -IncludeSecret } else { Save-Config }
	}
}
...
function Get-PlainPassword {
	...
	else { if ([string]::IsNullOrEmpty($State.EncPass)) { return '' }; try { $sec = ConvertTo-SecureString $State.EncPass; return (New-Object System.Net.NetworkCredential('', $sec)).Password } catch { return '' } }
}
...
function Get-Sha1Hex([Parameter(Mandatory=$true)][string]$Text){
...
function Release-MutexSafe($m){
	try { if ($m) { $m.ReleaseMutex() | Out-Null } } catch {}
...
function Ensure-SingleInstanceForConfig {
	...
	$mutexVar = $null
	if (-not (Acquire-NamedMutex -Name $key -MutexOut ([ref]$mutexVar))) {
		Show-WarnT 'message.config_already_running' @{ path = $cfgPath }
		exit 2
	}
...
function Encode-OcsPath([string]$path) {
	$norm = Normalize-SubPath $path
...
function Invoke-NcOcsFolderTreeRaw {
	...
	try {
		...
		return [pscustomobject]@{ StatusCode = $null; BodyRaw = $null; Json = $null }
	}
}
...
function Test-NcFolderExists([string]$subPath) {
	...
	if ($res -and $res.StatusCode -eq 404) {
		...
		# 404 but no exact "Folder not found" -> treat as unknown (NOT false)
		return $true
	}
	# Any other status or error -> unknown (NOT false)
	return $true
}
# Unified server status via /status.php (no auth)
function Get-NcServerStatus {
	if ([string]::IsNullOrWhiteSpace($State.Server)) {
		return [pscustomobject]@{ Reachable=$false; Maintenance=$false; Installed=$false; Json=$null }
	}
	try { [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12 } catch {}
...
function Test-NcOnlineAuthenticated {
	...
	if ($res -and $res.StatusCode -eq 200) {
		if ($res.Json -is [System.Array] -and $res.Json.Count -eq 0) { return $true }
	}
	return $false
}
function Ensure-ConfiguredFolderOrExit {
	...
	if ($ans -ne [System.Windows.Forms.DialogResult]::Yes) {
		...
		try {
			if ($script:mainMutex) { $script:mainMutex.ReleaseMutex() | Out-Null; $script:mainMutex.Dispose() }
		} catch {}
...
function Get-NcFolderChildren([string]$parentPath) {
	...
	if ($res -is [System.Array]) {
		return ($res | Where-Object { $_.PSObject.Properties.Name -contains 'basename' } | ForEach-Object { [string]$_.basename }) | Sort-Object
	}
	if ($res.PSObject.Properties.Name -contains 'ocs' -and ($res.ocs.data -is [System.Array])) {
		return ($res.ocs.data | Where-Object { $_.PSObject.Properties.Name -contains 'basename' } | ForEach-Object { [string]$_.basename }) | Sort-Object
	}
	return @()
}
...
function Join-SubPath([string]$base, [string]$child) {
	$base  = Normalize-SubPath $base
	$child = Normalize-SubPath $child
...
function Show-NcFolderPicker {
	...
	$getCP = { $curPathRef.Value }
	$setCP = { param($p) $curPathRef.Value = (Normalize-SubPath $p) }
	# --- UI ---
	$f = New-Object System.Windows.Forms.Form
	Apply-BrandIconToForm $f
	Hook-FormDpi $f
	$f.Text = (T 'title.folderpicker' @{ app = $AppName }); $f.StartPosition = 'CenterScreen'; $f.FormBorderStyle = 'FixedDialog'; $f.MaximizeBox = $false; $f.MinimizeBox = $false; $f.TopMost = $true; $f.Width = 520; $f.Height = 420; $f.KeyPreview = $true
	...
	$btnUp = New-Object System.Windows.Forms.Button; $btnUp.Text = (T 'button.up'); $btnUp.Left = 12; $btnUp.Top = 346; $btnUp.Width = 70
	$btnOpen = New-Object System.Windows.Forms.Button; $btnOpen.Text = (T 'button.open'); $btnOpen.Left = $btnUp.Left + $btnUp.Width + 8; $btnOpen.Top = 346; $btnOpen.Width = 80
	$btnSelect = New-Object System.Windows.Forms.Button; $btnSelect.Text = (T 'button.select'); $btnSelect.Width = 100; $btnSelect.Left = $f.ClientSize.Width - 220; $btnSelect.Top = 346; $btnSelect.Anchor = 'Bottom,Right'
	$btnCancel = New-Object System.Windows.Forms.Button; $btnCancel.Text = (T 'button.cancel'); $btnCancel.Width = 100; $btnCancel.Left = $f.ClientSize.Width - 110; $btnCancel.Top = 346; $btnCancel.Anchor = 'Bottom,Right'
	...
	$refresh = {
		...
		$lb.Items.Clear()
		$children = Get-NcFolderChildren $p
	...
	$openSelected = {
		...
		$newPath = Join-SubPath (& $getCP) $sel
		& $setCP $newPath
		& $refresh
	}
	# Up one level
	$btnUp.Add_Click({
		...
		$newP = if ($parts.Count -le 1) { '' } else { ($parts[0..($parts.Count-2)] -join '/') }
		& $setCP $newP
		& $refresh
	})
	# Open/double-click
	...
	$btnSelect.Add_Click({
		$sel = [string]$lb.SelectedItem
		$chosen = if ([string]::IsNullOrWhiteSpace($sel)) { (& $getCP) } else { Join-SubPath (& $getCP) $sel }
		$f.Tag = $chosen
	...
	# Enter/Esc behavior
	$f.AcceptButton = $btnSelect
	$f.CancelButton = $btnCancel
	$f.Add_KeyDown({
		param($s,$e)
		if ($e.KeyCode -eq 'Enter') { $btnSelect.PerformClick() }
		elseif ($e.KeyCode -eq 'Escape') { $btnCancel.PerformClick() }
	})
...
function New-StatusIcon([string]$label, [System.Drawing.Color]$color) {
    ...
    $g.SmoothingMode='AntiAlias'; $g.PixelOffsetMode='Half'; $g.TextRenderingHint='AntiAliasGridFit'
    ...
    [single]$offsetY = 6
    [single]$offsetX = if ($txt.Length -eq 2) { 4 } else { 3 }
    [single]$height  = [Math]::Max(1, 16 - $offsetY)
    ...
    try {
        $hicon = $bmp.GetHicon()
        $icoTmp = [System.Drawing.Icon]::FromHandle($hicon)
        $icon   = [System.Drawing.Icon]$icoTmp.Clone()
        $icoTmp.Dispose()
    } finally {
        if ($hicon -ne [IntPtr]::Zero) { [void][Nc.Win32]::DestroyIcon($hicon) }
        $g.Dispose(); $brush.Dispose(); $font.Dispose(); $sf.Dispose(); $bmp.Dispose()
    }
    return $icon
}
...
function Save-IconFromBitmap([System.Drawing.Bitmap]$bmp, [string]$icoPath) {
    ...
    try {
        ...
        try {
            ...
            try { $icoTmp.Save($fs) } finally { $fs.Close() }
        } finally {
...
function Set-MP2LabelExactExact {
	...
	$keyName = "##$Server@ssl#remote.php#dav#files#$User$frag"
	try {
		# Open MP2 base with write access
		$cu  = [Microsoft.Win32.Registry]::CurrentUser
		$mp2 = $cu.CreateSubKey('Software\Microsoft\Windows\CurrentVersion\Explorer\MountPoints2', $true)
		...
		try {
			...
			if ($key) {
				if ([string]::IsNullOrWhiteSpace($Label)) {
					try { $key.DeleteValue('_LabelFromReg', $false) } catch {}
				} else {
					$key.SetValue('_LabelFromReg', $Label, [Microsoft.Win32.RegistryValueKind]::String)
				}
				$key.Close()
			}
		} catch {}
	...
	try { Refresh-AllExplorerViews } catch {}
}
	...
	foreach ($name in $keys) {
		...
		if (Test-Path -LiteralPath $p) {
			try {
				...
				if ($val -and ($val -eq $Label)) { return $true }
			} catch {}
			...
			if (Test-Path -LiteralPath $s) {
				try {
					...
					if ($val2 -and ($val2 -eq $Label)) { return $true }
				} catch {}
			}
		}
	}
...
function Set-DriveIconHKCU {
	[CmdletBinding()]
	param(
		[Parameter(Mandatory)][string]$DriveLetter,	# e.g. 'F:'
		[Parameter(Mandatory)][string]$IconPath		# full path to .ico
	)
	# Validate
...
function Remove-DriveIconHKCU {
	...
	try {
		$cu   = [Microsoft.Win32.Registry]::CurrentUser
		$base = $cu.OpenSubKey('Software\Classes\Applications\Explorer.exe\Drives', $true)
		if ($base) {
			try { $base.DeleteSubKeyTree($dl, $false) } catch {}
			$base.Close()
		}
	} catch {}
	try { Refresh-ShellIcons } catch {}
...
function Test-DriveIconApplied {
	...
	if (-not ($DriveLetter -match '^[A-Za-z]:$')) { return $false }
...
function Apply-WebDavBrandingFrom-State {
	...
	if ([string]::IsNullOrWhiteSpace($State.Server) -or
		[string]::IsNullOrWhiteSpace($State.User)   -or
		-not (Test-ValidDrive $State.Drive)) { return }
	# Ensure .ico file exists locally; fetch immediately if missing
	$icoPath = Get-FavIconFilePath
	if (-not (Test-Path -LiteralPath $icoPath)) {
		if ($script:ServerFaviconBmp) {
			[void](Save-IconFromBitmap $script:ServerFaviconBmp $icoPath)
		} else {
			[void](Fetch-ServerFavicon $State.Server)
		}
	}
...
function Test-DriveAccessible {
	try {
		...
		if (-not (Test-DriveMatchesDesired))     { return $false }
		Get-ChildItem -LiteralPath ("{0}\" -f $State.Drive) -Force -ErrorAction Stop | Out-Null
		return $true
	} catch { return $false }
}
...
function Map-Drive {
	...
	if (-not [string]::IsNullOrWhiteSpace($State.SubPath)) {
		...
		if (-not (Test-NcFolderExists $State.SubPath)) {
			...
			$State.Label   = ''
			Save-Config
			$ans = Ask-YesNoWarnT 'prompt.folder_no_longer_exists_choose_new' @{ folder = $gone }
	...
	if ($occupied -and (Test-DriveMatchesDesired)) {
		$ok = Test-DriveAccessible
		if ($ok) { try { Apply-WebDavBrandingFrom-State } catch {} }
		return $ok
	}
	Unmap-DriveIfOurs
	[int]$rc = 0
	if (New-WebDavMapSecure -drive $State.Drive -unc $unc -user $State.User -pass $pass -LastError ([ref]$rc)) {
		if (Test-DriveAccessible) {
			try { Apply-WebDavBrandingFrom-State } catch {}
			return $true
		}
	} else {
		$script:tray.Text = "mapping failed (rc=$rc)"
	}
	return $false
}
...
function Unmap-DriveIfOurs {
	...
	param(
		...
		[string]$SubPath = $State.SubPath
	)
	if (-not (Test-ValidDrive $drive)) { return }
	if (-not $Force) {
		if (-not ($script:mapMutexOwned -or (Test-DriveMatchesDesired))) { return }
	}
	# disconnect letter (as du schon hast) ...
	...
	# also by UNC ...
	try { $unc = if ($Server -and $User) { Build-Unc $Server $User $SubPath } else { $null }
		if ($unc) { [void][Nc.NetUse]::WNetCancelConnection2($unc, $flags, $true) }
	} catch {}
	...
	try {
		if (Get-PSDrive -Name $drive.Substring(0,1) -ErrorAction SilentlyContinue) {
			Start-Process -FilePath "$env:SystemRoot\System32\net.exe" `
				-ArgumentList ("use {0} /delete /y" -f $drive) -WindowStyle Hidden -Wait | Out-Null
		}
	} catch {}
	...
	try { Refresh-ExplorerDriveRemoval $drive } catch {}
}
function Wait-DriveFullyUnmapped {
	[CmdletBinding()]
	param(
		[string]$Drive,         # e.g. 'Z:'
		[int]$TimeoutMs = 3000  # total wait budget
	)
	...
	while ($sw.ElapsedMilliseconds -lt $TimeoutMs) {
		$exists = Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue |
			Where-Object { ('{0}:' -f $_.Name) -ieq $Drive } |
...
function Ensure-MapMutex {
	$name = "Local\${AppName}:map:{0}:{1}:{2}:{3}" -f (
		($State.Drive.ToUpper()),
		$State.Server.ToLower(),
		$State.User.ToLower(),
		(Normalize-SubPath $State.SubPath).ToLower()
	)
	if ($script:mapMutexName -ne $name -and $script:mapMutexObj) {
		try { $script:mapMutexObj.ReleaseMutex() | Out-Null } catch {}
		try { $script:mapMutexObj.Dispose() } catch {}
		$script:mapMutexOwned = $false
		$script:mapMutexObj   = $null
	}
	if (-not $script:mapMutexObj) {
		$owned = $false
		$obj   = New-Object System.Threading.Mutex($true, $name, [ref]$owned)
		$script:mapMutexObj   = $obj
		$script:mapMutexName  = $name
		$script:mapMutexOwned = $owned
	}
}
function Test-DriveMatchesDesired {
	try {
		if (-not (Test-ValidDrive $State.Drive) -or
			[string]::IsNullOrWhiteSpace($State.Server) -or
			[string]::IsNullOrWhiteSpace($State.User)) { return $false }
		$pd = Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue |
			  Where-Object { ('{0}:' -f $_.Name) -ieq $State.Drive } |
			  Select-Object -First 1
		...
		$svr = [regex]::Escape($State.Server)
		$usr = [regex]::Escape($State.User)
		$norm = Normalize-SubPath $State.SubPath
		$subRx = ''
		if ($norm) {
			$parts = $norm -split '/'
			$parts = $parts | ForEach-Object { [regex]::Escape($_) }
			$subRx = '\\' + ($parts -join '\\')
		}
...
function Refresh-NeedsSetup {
	$script:NeedsSetup = (
		[string]::IsNullOrWhiteSpace($State.Server) -or
		[string]::IsNullOrWhiteSpace($State.User)   -or
		-not (Test-ValidDrive $State.Drive)
	)
}
...
function Set-TrayState([string]$Text=$null, $Icon = $null){
	if (-not $script:tray) { return }	# no tray yet
	if ($Icon)  { $script:tray.Icon = $Icon }
	if ($Text)  { $script:tray.Text = $Text }
}

# ================================================================
#	Dialogs (passphrase prompt, drive-in-use)
# ================================================================
# One-time global flag (keep if not present)
...
function Show-CustomBalloon([string]$Title,[string]$Text,[int]$TimeoutMs=2200){
	...
	if ($Title -is [array]) { $Title = ($Title -join ' ') }
	...
	$fontTitle = New-Object System.Drawing.Font($UiFontFamily,12,$UiFontStyle)
	$fontText  = New-Object System.Drawing.Font($UiFontFamily,8,$UiFontStyle) #[System.Drawing.FontStyle]::Regular)
	...
	$wText = [Math]::Max($szTitle.Width,$szText.Width)
	$wIcon = if($IconImage){ $iconSz + $pad }else{ 0 }
	$w = [int]($pad + $wIcon + $wText + $pad)
	$h = [int]($pad + [Math]::Max($iconSz, $szTitle.Height + $gap + $szText.Height) + $pad)
	# enforce bigger popup footprint when icon is large
	if($w -lt ( $pad + $iconSz + 160 )){ $w = $pad + $iconSz + 160 }
	if($h -lt ( $pad + $iconSz + $pad )){ $h = $pad + $iconSz + $pad }
	# --- form ---
	$f = New-Object System.Windows.Forms.Form
	...
	$f.Add_Paint({
		...
		$wClient = [int]$s.ClientSize.Width
		$hClient = [int]$s.ClientSize.Height
		$pad     = [int]$t.Padding
		$iconSz  = [int]$t.IconSz
		$gap     = [int]$t.Gap
		$x       = [int]$pad
		...
		if($t.IconImage){
			try { $g.DrawImage($t.IconImage,$x,$y,$iconSz,$iconSz) } catch {}
			$x = [int]($x + $iconSz + $pad)
		}
		...
		[System.Windows.Forms.TextRenderer]::DrawText(
			$g,$t.Text,$t.FontText,
			(New-Object System.Drawing.Rectangle($x,[int]$y,[int]$remainW,$s.ClientSize.Height)),
			[System.Drawing.Color]::DimGray,
			$t.Fmt
		)
		# tiny 'Click to close' hint
		$hint = 'Click to close'
		$hintFont = New-Object System.Drawing.Font($UiFontFamily,7,[System.Drawing.FontStyle]::Regular)
		$szHint = [System.Windows.Forms.TextRenderer]::MeasureText($hint,$hintFont)
		$hx = [int]($wClient - $pad - [int]$szHint.Width)
		$hy = [int]($hClient - $pad - [int]$szHint.Height)
	...
	# Ballon as Chrome-Style Toast:
	$screen = [System.Windows.Forms.Screen]::PrimaryScreen.WorkingArea
	$posX = $screen.Right - $w - 10
	$posY = $screen.Bottom - $h - 10

	$f.Location = New-Object System.Drawing.Point($posX,$posY)
	$f.Location = New-Object System.Drawing.Point($posX,$posY)
	# Close on click / mousedown / ESC (sender-basiert, kein $myForm, kein GetNewClosure)
	$f.Add_Click({ param($s,$e) try{ $s.Close() }catch{} })
	$f.Add_MouseDown({ param($s,$e) try{ $s.Close() }catch{} })
	$f.KeyPreview = $true
	$f.Add_KeyDown({ param($s,$e) if($e.KeyCode -eq 'Escape'){ try{ $s.Close() }catch{} } })
	# Auto-close via WinForms timer (UI-thread; safe with message pump)
	$t = New-Object System.Windows.Forms.Timer
	$t.Interval = [Math]::Max(500,$TimeoutMs)
	...
	$t.Add_Tick((
		{
			...
			try{ if($myForm -and -not $myForm.IsDisposed){ $myForm.Close() } }catch{}
		}
	).GetNewClosure())
	$script:BalloonTimer = $t
	$script:BalloonForm  = $f
	...
	$f.Add_FormClosed((
		{
			...
			try{
				if($myForm.Tag){
					if($myForm.Tag.Path){ $myForm.Tag.Path.Dispose(); $myForm.Tag.Path=$null }
					if($myForm.Tag.FontTitle){ $myForm.Tag.FontTitle.Dispose(); $myForm.Tag.FontTitle=$null }
					if($myForm.Tag.FontText){  $myForm.Tag.FontText.Dispose();  $myForm.Tag.FontText=$null  }
					if($myForm.Tag.IconImage){ $myForm.Tag.IconImage.Dispose(); $myForm.Tag.IconImage=$null }
				}
			}catch{}
			try{ $myForm.Dispose() }catch{}
			# reset globals only if they still point to THIS instance
			try{
				if($script:BalloonForm -eq $myForm){ $script:BalloonForm = $null }
				if($script:BalloonTimer -eq $myTimer){ $script:BalloonTimer = $null }
			}catch{}
		}
	).GetNewClosure())
	try{ $t.Start() }catch{}
	$f.Add_Shown({ param($s,$e); try{ $s.Invalidate() }catch{} })
	$f.Show()
	try{ $f.Activate(); $f.BringToFront(); [void]$f.Focus() }catch{}
}
# Strip leading "<AppName>: " and optional "[D-Z]: "
function Get-TrayStatusCore([string]$text){
	if([string]::IsNullOrWhiteSpace($text)){ return '' }
	$pat = '^' + [regex]::Escape($AppName) + ':\s*(?:[D-Z]:\s*)?'
...
function Prompt-Passphrase([string]$title, [switch]$Confirm, [switch]$ExitOnCancel) {
	...
	$txt2 = $null
	if ($Confirm) {
		$lbl2 = New-Object System.Windows.Forms.Label; $lbl2.Text = (T 'label.passphrase_confirm'); $lbl2.AutoSize = $true; $lbl2.Left = 12; $lbl2.Top = 56; $txt2 = New-Object System.Windows.Forms.TextBox; $txt2.Left = 160; $txt2.Top = 54; $txt2.Width = 230; $txt2.UseSystemPasswordChar = $true
	}
	$ok = New-Object System.Windows.Forms.Button; $ok.Text = (T 'button.ok'); $ok.Width = 100; $ok.Left = $f.ClientSize.Width - 216; $ok.Top = $f.ClientSize.Height - 40; $ok.Anchor = 'Bottom,Right'
	$ok.Add_Click({
		if ($Confirm -and ($txt.Text -ne $txt2.Text)) { Show-WarnT 'message.passphrase_mismatch'; return }
		$f.DialogResult = [System.Windows.Forms.DialogResult]::OK
		$f.Close()
	})
	$ca = New-Object System.Windows.Forms.Button; $ca.Text = (T 'button.cancel'); $ca.Width = 100; $ca.Left = $f.ClientSize.Width - 108; $ca.Top = $f.ClientSize.Height - 40; $ca.Anchor = 'Bottom,Right'
	...
	$f.Add_KeyDown({
		param($s,$e)
		if ($e.KeyCode -eq 'Enter') { $ok.PerformClick() }
		elseif ($e.KeyCode -eq 'Escape') { $ca.PerformClick() }
	})
...
function Ensure-PortablePass([switch]$Interactive) {
	...
	if ($null -eq $pp) {
		...
		try {
			if ($script:mainMutex) {
				$script:mainMutex.ReleaseMutex() | Out-Null
				$script:mainMutex.Dispose()
			}
		} catch {}
		try { if ($script:tray) { $script:tray.Visible=$false; $script:tray.Dispose() } } catch {}
		[System.Windows.Forms.Application]::Exit()
	...
	try {
		$script:PlainPassCache = Unprotect-PortableSecret $pp $SecretPath
		return $true
	} catch {
		Show-WarnT 'message.passphrase_wrong_or_corrupt'
		$script:AskedPassphrase = $false
		return $false
	}
}
...
function New-Shortcut($lnkPath, $scriptPath, [string]$IconPath = $null) {
	...
	# prefer custom icon if provided, fallback stock shell icon
	if ($IconPath -and (Test-Path -LiteralPath $IconPath)) { $sc.IconLocation = "$IconPath,0" } else { $sc.IconLocation = "$env:SystemRoot\system32\shell32.dll,44" }
...
function Ensure-Shortcut($which, [bool]$enable) {
	...
	else { if (Test-Path $lnk) { Remove-Item $lnk -Force } }
}
function Install-Self {
	[CmdletBinding()]
	param([switch]$FromInstaller)
	...
	try {
		...
		if (-not $FromInstaller) {
			...
			if ($ans -eq [System.Windows.Forms.DialogResult]::Yes) {
				...
				if (Test-Path $SecretPath) {
					$pp = Prompt-Passphrase (T 'title.passphrase_query' @{ app = $AppName })
					if ($pp) {
						try { $plain = Unprotect-PortableSecret $pp $SecretPath } catch {}
					}
				} elseif ($script:PlainPassCache) {
					$plain = $script:PlainPassCache
				}
				# 2) convert plaintext to DPAPI blob for installed mode
				$encVal = ''
				if ($plain) {
					$sec    = ConvertTo-SecureString $plain -AsPlainText -Force
					$encVal = ($sec | ConvertFrom-SecureString)
				}
		...
		New-Shortcut -lnkPath (Get-StartMenuShortcutPath) -scriptPath $InstallBin -IconPath $iconIcoPath
		New-Shortcut -lnkPath (Get-DesktopShortcutPath)   -scriptPath $InstallBin -IconPath $iconIcoPath
		...
		if (-not $FromInstaller) {
			# relaunch installed (hidden) and exit this instance
...
function Write-PortableLaunchers {
	[CmdletBinding()]
	param(
		[Parameter(Mandatory=$true)][string]$Destination, # target folder (exists)
		[Parameter(Mandatory=$true)][string]$ScriptName,  # e.g. "NDT.ps1"
		[Parameter(Mandatory=$true)][string]$SpdxFrom     # read SPDX lines from this file
	)
	...
	$vbsBody = @(
		...
		("ps1  = fso.BuildPath(here, ""{0}"")" -f $ScriptName),
		"",
	...
	$cmdBody = @(
		"@echo off",
		("wscript.exe ""%~dp0{0}.vbs""" -f $AppNameShort)
	)
...
function Export-To-Portable {
	...
	if (-not $SkipConfirm) {
		$ansPre = Ask-YesNoQuestT 'prompt.export2portable' @{ appshort = $AppNameShort; app = $AppName }
		if ($ansPre -ne [System.Windows.Forms.DialogResult]::Yes) { return }
	}
	...
	try {
		...
		if (-not $scriptPath) {
			...
			$ofd.Title  = ('Select {0}.ps1' -f $AppNameShort)
			$ofd.Filter = 'PowerShell script (*.ps1)|*.ps1'
			$ofd.InitialDirectory = (Get-Location).Path
			if ($ofd.ShowDialog() -ne 'OK') { return }
			$scriptPath = $ofd.FileName
		}
		if ([string]::IsNullOrWhiteSpace($scriptPath) -or -not (Test-Path -LiteralPath $scriptPath)) {
			$display = if ($scriptPath -ne $null -and $scriptPath -ne '') { $scriptPath } else { '<null>' }
			throw ("Cannot resolve script path: {0}" -f $display)
		}
		$scriptName = [System.IO.Path]::GetFileName($scriptPath)
		$hereAbs    = [System.IO.Path]::GetDirectoryName($scriptPath)
		...
		$dlg.Description  = 'Choose destination folder for portable package (select existing portable folder to update in place)'
		$dlg.SelectedPath = $defaultSel
		...
		if (Test-IsPortableFolder -Path $chosen) {
			$dstRoot = $chosen
			$updateInPlace = $true
			Write-Verbose "Detected existing portable folder -> update in place."
		}
		# Case 2: otherwise use <chosen>\<AppName>Portable; if that is portable, also update in place
		if (-not $dstRoot) {
			$maybeRoot = Join-Path $chosen ("{0}Portable" -f $AppName)
			if (Test-IsPortableFolder -Path $maybeRoot) {
				$dstRoot = $maybeRoot
				$updateInPlace = $true
				Write-Verbose "Detected existing portable folder at nested path -> update in place."
			} else {
				$dstRoot = $maybeRoot
				Write-Verbose ("Will create new portable package at: {0}" -f $dstRoot)
			}
		}
		# --- CASE A: same folder as script -> write launchers only (bare helper) ---
		if ($chosen -ieq $hereAbs) {
			Write-Verbose "In-place launchers in script folder (bare mode)..."
			Write-PortableLaunchers -Destination $hereAbs -ScriptName $scriptName -SpdxFrom $scriptPath
			Show-InfoT 'message.portable_launchers_inplace' @{ path = $hereAbs }
			return
		}
			...
			if (-not $updateInPlace) {
				...
				if ($count -gt 0) {
					...
					if ($ans2 -eq [System.Windows.Forms.DialogResult]::Yes) {
						try { Remove-Item -LiteralPath $dstRoot -Recurse -Force -ErrorAction SilentlyContinue } catch {}
						New-Item -ItemType Directory -Path $dstRoot -Force | Out-Null
					} else {
						$dstRoot = Join-Path $chosen ("{0}Portable_{1}" -f $AppName, (Get-Date -Format 'yyyyMMdd_HHmmss'))
						New-Item -ItemType Directory -Path $dstRoot -Force | Out-Null
					}
				}
			}
		...
		Write-Verbose ("Copy script ->  {0}" -f $dstPs1)
		...
		$srcI18n = Join-Path $hereAbs 'i18n'
		$dstI18n = Join-Path $dstRoot 'i18n'
		if (-not (Test-Path -LiteralPath $dstI18n)) {
			New-Item -ItemType Directory -Path $dstI18n -Force | Out-Null
		}
		if (Test-Path -LiteralPath $srcI18n) {
			try {
				Copy-Item -Path (Join-Path $srcI18n '*') -Destination $dstI18n -Recurse -Force -ErrorAction SilentlyContinue
			} catch {}
		}
		...
		if (-not $bare -and -not $updateInPlace) {
			...
			if ([string]::IsNullOrEmpty($pw)) {
				...
				$ok=New-Object Windows.Forms.Button; $ok.Text = (T 'button.ok'); $ok.Left=170; $ok.Top=60; $ok.Width=70
				$ca=New-Object Windows.Forms.Button; $ca.Text = (T 'button.cancel'); $ca.Left=250; $ca.Top=60; $ca.Width=70
			...
			if     ($ppRes -is [string]) { $ppVal = $ppRes }
			elseif ($ppRes -and ($ppRes.PSObject.Properties.Name -contains 'Value')) { $ppVal = $ppRes.Value }
			...
			# Update path (or bare): never delete config/secret; just leave them as-is
			Write-Verbose "Portable update/bare: keeping existing *_portable.json and *_secret.dat unchanged."
		}
		...
		Write-Verbose ("ERROR at: {0}" -f ($_.InvocationInfo.PositionMessage))
		Show-ErrorT 'message.export_failed' @{ err = $_.Exception.Message }
	}
	finally {
...
function Export-InstalledConfig([switch]$SkipConfirm){
	...
	try {
		...
		$dlg.Title    = (T 'title.export_config')
		$dlg.Filter   = 'JSON (*.json)|*.json'
		$dlg.FileName = ('{0}_installed.json' -f $AppName)
		$dlg.InitialDirectory = [Environment]::GetFolderPath('Desktop')
		if ($dlg.ShowDialog() -ne 'OK') { return }
		# Object that includes normal keys + DPAPI secret as 'DPAPI'
		$payload = New-InstalledExportObject
		$payload | ConvertTo-Json | Set-Content -Path $dlg.FileName -Encoding UTF8
...
# ================================================================
#	Settings dialog (single form + handlers)
# ================================================================
function Show-SettingsDialog([string]$ConflictDrive){
	# Singleton guard: if settings dialog is already open, just focus it and return
	if ($script:IsSettingsOpen -and $script:SettingsForm -and (-not $script:SettingsForm.IsDisposed)) {
		...
		# Return a safe default. Callers that wait for DialogResult::OK, will see this as "not ready yet".
		return [System.Windows.Forms.DialogResult]::Cancel
	}
	# mark dialog as open right away, so re-entrant calls while we build UI won't spawn duplicates
	$script:IsSettingsOpen = $true
	$f = New-Object System.Windows.Forms.Form
	Apply-BrandIconToForm $f
	Hook-FormDpi $f
	$f.Text = (T 'title.settings_dialog' @{ app = $AppName }); $f.StartPosition = 'CenterScreen'; $f.FormBorderStyle = 'FixedDialog'; $f.MaximizeBox = $false; $f.MinimizeBox = $false; $f.TopMost = $true; $f.Width = 600; $f.Height = 440
	# Ensure $State has LangPref property so we can read/write it safely
	if (-not ($State.PSObject.Properties.Name -contains 'LangPref')) { $State | Add-Member -NotePropertyName 'LangPref' -NotePropertyValue '' -Force }
	# Mode badge
	$lblMode = New-Object System.Windows.Forms.Label
	if ($PortableMode) { $lblMode.Text = (T 'mode.portable'); $lblMode.ForeColor = [System.Drawing.Color]::ForestGreen; $lblMode.Font = New-Object System.Drawing.Font($UiFontFamily, 10, $UiFontStyle) }
	else { $lblMode.Text = (T 'mode.installed'); $lblMode.ForeColor = [System.Drawing.Color]::SteelBlue; $lblMode.Font = New-Object System.Drawing.Font($UiFontFamily, 9, $UiFontStyle) }
	$lblMode.AutoSize=$true; $lblMode.Left=12; $lblMode.Top=10
	$lblSrv = New-Object Windows.Forms.Label; $lblSrv.Text = (T 'label.server'); $lblSrv.Left = 12; $lblSrv.Top = 40; $lblSrv.AutoSize=$true
	$txtSrv = New-Object Windows.Forms.TextBox; $txtSrv.Left = 190; $txtSrv.Top = 38; $txtSrv.Width = 370; $txtSrv.Text = $State.Server
	$lblUsr = New-Object Windows.Forms.Label; $lblUsr.Text = (T 'label.user');   $lblUsr.Left = 12; $lblUsr.Top = 70; $lblUsr.AutoSize=$true
	$txtUsr = New-Object Windows.Forms.TextBox; $txtUsr.Left = 190; $txtUsr.Top = 68; $txtUsr.Width = 370; $txtUsr.Text = $State.User
	# Small images next to Server/User
	$picFav = New-Object System.Windows.Forms.PictureBox; $picFav.Width = 24; $picFav.Height = 24; $picFav.SizeMode = 'Zoom'; $picFav.Left = $txtSrv.Left - 28; $picFav.Top = $txtSrv.Top - 2
	$picAv = New-Object System.Windows.Forms.PictureBox; $picAv.Width = 24; $picAv.Height = 24; $picAv.SizeMode = 'Zoom'; $picAv.Left = $txtUsr.Left - 28; $picAv.Top = $txtUsr.Top - 2
	# Debounce timers
	$favTimer = New-Object System.Windows.Forms.Timer; $favTimer.Interval = 500; $favTimer.add_Tick({ $favTimer.Stop(); $bmp = Fetch-ServerFavicon ($txtSrv.Text.Trim()); if ($bmp) { $picFav.Image = $bmp } else { $picFav.Image = $null } })
	$avTimer = New-Object System.Windows.Forms.Timer; $avTimer.Interval = 500; $avTimer.add_Tick({ $avTimer.Stop(); $bmp = Fetch-UserAvatar ($txtSrv.Text.Trim()) ($txtUsr.Text.Trim()); if ($bmp) { $picAv.Image = $bmp } else { $picAv.Image = $null } })
	$txtSrv.add_TextChanged({ $favTimer.Stop(); $favTimer.Start(); $avTimer.Stop(); $avTimer.Start() })
	$txtUsr.add_TextChanged({ $avTimer.Stop(); $avTimer.Start() })
	# Initial fetch (existing config)
	[void](Fetch-ServerFavicon ($txtSrv.Text.Trim()))
	if ($script:ServerFaviconBmp) { $picFav.Image = $script:ServerFaviconBmp }
	[void](Fetch-UserAvatar ($txtSrv.Text.Trim()) ($txtUsr.Text.Trim()))
	if ($script:UserAvatarBmp) { $picAv.Image = $script:UserAvatarBmp }
	# Password
	$lblPwd = New-Object Windows.Forms.Label; $lblPwd.Text=T 'label.app_password'; $lblPwd.Left = 12; $lblPwd.Top = 100; $lblPwd.AutoSize=$true
	# Small "?" help button next to the password box
	$btnPwdHelp = New-Object Windows.Forms.Button; $btnPwdHelp.Text = '?'; $btnPwdHelp.Width = 24; $btnPwdHelp.Left = 162; $btnPwdHelp.Top = 98
	$txtPwd = New-Object Windows.Forms.TextBox; $txtPwd.Left = 190; $txtPwd.Top = 98; $txtPwd.Width = 250; $txtPwd.UseSystemPasswordChar=$true
	# Encrypt button (right of password TextBox)
	$btnEncrypt = New-Object Windows.Forms.Button; $btnEncrypt.Text = (T 'button.encrypt'); $btnEncrypt.Width = 110; $btnEncrypt.Left = $txtPwd.Left + $txtPwd.Width + 10; $btnEncrypt.Top = $txtPwd.Top - 1
	...
	$btnPwdHelp.Add_Click(({
		...
		# call captured closure + captured AppName/Parent
		[void](Show-HelpT -TitleKey 'title.app_password_help' -TitleVars @{ app = $appLocal } -BodyKey  'message.app_password_help' -BodyVars @{ url = $url; app = $appLocal } -Url $url -Width 640 -Height 320 -Parent $f)
	}).GetNewClosure())
	$f.Controls.Add($btnPwdHelp)
	# Subfolder (read-only -> only selection via picker)
	$lblSub = New-Object Windows.Forms.Label; $lblSub.Text = (T 'label.subfolder'); $lblSub.Left = 12; $lblSub.Top = 130; $lblSub.AutoSize = $true
	$txtSub = New-Object Windows.Forms.TextBox; $txtSub.Left = 190; $txtSub.Top = 128; $txtSub.Width = 370; $txtSub.Text = $State.SubPath; $txtSub.ReadOnly = $true; $txtSub.BackColor = [System.Drawing.SystemColors]::ControlLightLight
	...
	$OpenSubfolderPicker = {
		...
		$State.Server = $txtSrv.Text.Trim()
		$State.User   = $txtUsr.Text.Trim()
		# remember old subfolder to detect a real change
		$oldSub = $txtSub.Text
		$pick = Show-NcFolderPicker
		if ($pick -ne $null) {
			$oldDefault = (& $ComputeDefaultLabel $oldSub $txtUsr.Text.Trim() $txtSrv.Text.Trim())
			$newDefault = (& $ComputeDefaultLabel $pick $txtUsr.Text.Trim() $txtSrv.Text.Trim())
			# user actually changed folder -> suggest new label
			if ($pick -ne $oldSub) { $txtDisp.Text = $newDefault; $auto.LastAuto = $newDefault }
			# same folder -> only update label if user hasn't customized manually
			else { if ([string]::IsNullOrWhiteSpace($txtDisp.Text) -or ($txtDisp.Text -eq $oldDefault) -or ($txtDisp.Text -eq $auto.LastAuto)) { $txtDisp.Text = $newDefault; $auto.LastAuto = $newDefault } }
			$txtSub.Text = $pick
		}
	}
	# shared browse-state flag (controls picker availability)
	$browseState = @{ CanBrowse = $false }
	# Small tooltip for disabled controls
	$tip = New-Object System.Windows.Forms.ToolTip
	# Small tooltip helper (shared for all controls)
	$tip.AutoPopDelay = 8000  # how long the tooltip stays visible (ms)
	$tip.InitialDelay = 500   # delay before first show (ms)
	$tip.ReshowDelay  = 200   # delay for subsequent shows (ms)
	$tip.ShowAlways   = $true # also show for disabled controls
	# Hover tooltips for Server/User/Password
	$tip.SetToolTip($txtSrv, (T 'tip.enter_server'))
	$tip.SetToolTip($txtUsr, (T 'tip.enter_user'))
	$tip.SetToolTip($btnPwdHelp, (T  'title.app_password_help'))
	...
	$RefreshPasswordState = {
		...
		$browseState.CanBrowse = $localCanBrowse
		# Update tooltip text
		if ($localCanBrowse) { $tip.SetToolTip($txtSub, $null) }
		else { $tip.SetToolTip($txtSub, (T 'tip.enter_password_and_encrypt')) }
		# Toggle Encrypt/Clear UI and show info text when a secret exists
		if ($PortableMode) {
			$btnEncrypt.Text = $(if ($hasEncPortable) { (T 'button.delete_secrets') } else { (T 'button.encrypt') })
			if ($hasEncPortable) {
				# Show info message instead of password dots
				$msg = (T 'password.encrypted_secret')
				$txtPwd.ReadOnly = $true
				$txtPwd.Enabled  = $true
				$txtPwd.UseSystemPasswordChar = $false
				$txtPwd.ForeColor = [System.Drawing.SystemColors]::GrayText
				$txtPwd.Text = $msg
				$txtPwd.Tag  = 'info'
			} else {
				# Back to normal editable password field
				if ($txtPwd.Tag -eq 'info') { $txtPwd.Text = '' }
				$txtPwd.Tag  = $null
				$txtPwd.ReadOnly = $false
				$txtPwd.Enabled  = $true
				$txtPwd.UseSystemPasswordChar = $true
				$txtPwd.ForeColor = [System.Drawing.SystemColors]::WindowText
			}
		} else {
			$btnEncrypt.Text = $(if ($hasEncInstalled) { (T 'button.clear') } else { (T 'button.encrypt') })
			if ($hasEncInstalled) {
				$msg = (T 'password.encrypted_dpapi')
				$txtPwd.ReadOnly = $true
				$txtPwd.Enabled  = $true
				$txtPwd.UseSystemPasswordChar = $false
				$txtPwd.ForeColor = [System.Drawing.SystemColors]::GrayText
		...
		$pwEmpty  = ([string]::IsNullOrWhiteSpace($txtPwd.Text) -or $txtPwd.Tag -eq 'info')
		$needPwd  = (-not $hasEncPortable -and -not $hasEncInstalled)
		if ($needPwd -and $pwEmpty) {
			$tip.SetToolTip($txtPwd, (T 'tip.enter_password'))
		} else {
			$tip.SetToolTip($txtPwd, $null)
		}
	}
	# Clicking the readonly path box
	$txtSub.add_MouseDown({
		# open picker directly
		if ($browseState.CanBrowse) { try { & $OpenSubfolderPicker } catch {} }
		# show enforced tooltip popup near the box for ~4s
		else { try { $msg = (T 'tip.enter_password_and_encrypt'); $tip.Show($msg, $txtSub, 0, $txtSub.Height + 2, 4000) } catch {} }
		# move focus away so txtSub does not look "active"
		try { $btnEncrypt.Focus() } catch {}
	})
	...
	$btnEncrypt.Add_Click({
		if ($PortableMode) {
			...
			if (Test-Path $SecretPath) {
				...
				# Encrypt now
				if ([string]::IsNullOrWhiteSpace($txtPwd.Text)) { Show-InfoT 'message.enter_password_first'; return }
				$pp = Prompt-Passphrase (T 'title.passphrase_set' @{ app = $AppName }) -Confirm
				if ([string]::IsNullOrWhiteSpace($pp)) { return }
				try {
					Protect-PortableSecret $txtPwd.Text $pp $SecretPath
					# Keep plaintext for this session so Browse works without re-prompt
					$script:PlainPassCache  = $txtPwd.Text
			...
			if (-not [string]::IsNullOrEmpty($State.EncPass)) {
				$ans = Ask-YesNoQuestT 'prompt.clear_stored_password'
				if ($ans -ne [System.Windows.Forms.DialogResult]::Yes) { return }
				Clear-PlainPassword # wipe secret everywhere + persist
				$txtPwd.Text = ''   # reset UI textbox immediately
			} else {
				if ([string]::IsNullOrWhiteSpace($txtPwd.Text)) { Show-InfoT 'message.enter_password_first'; return }
				try { Set-PlainPassword $txtPwd.Text } catch { Show-ErrorT 'message.store_password_failed' @{ err = $_.Exception.Message }; return }
			}
		}
		& $RefreshPasswordState
	})
	# Keep UI reactive if user edits the field
	$txtPwd.add_TextChanged({ & $RefreshPasswordState })
	# Initial state (now $btnBrowse exists)
	& $RefreshPasswordState
	# Drive selector
	$lblDrv = New-Object Windows.Forms.Label; $lblDrv.Text = (T 'label.drive_letter'); $lblDrv.Left = 12; $lblDrv.Top = 160; $lblDrv.AutoSize = $true
	$cmb = New-Object Windows.Forms.ComboBox; $cmb.Left = 190; $cmb.Top = 158; $cmb.Width = 60; $cmb.DropDownStyle='DropDownList'
	# Display name
	$lblDisp = New-Object Windows.Forms.Label; $lblDisp.Text = (T 'label.display_name'); $lblDisp.Left = 12; $lblDisp.Top = 190; $lblDisp.AutoSize=$true
	$txtDisp = New-Object Windows.Forms.TextBox; $txtDisp.Left = 190; $txtDisp.Top = 188; $txtDisp.Width = 370; $txtDisp.Text = $State.Label
	# Initial prefill for display name if empty (and remember that it was auto-filled)
	if ([string]::IsNullOrWhiteSpace($txtDisp.Text)) {
		$txtDisp.Text = (& $ComputeDefaultLabel $txtSub.Text $txtUsr.Text.Trim() $txtSrv.Text.Trim())
		$auto.LastAuto = $txtDisp.Text
	}
	# Keep Display name in sync while user types Server/User, but only if user did not customize it (i.e., equals last auto or empty).
	$txtSrv.add_TextChanged({
		$newDefault = (& $ComputeDefaultLabel $txtSub.Text $txtUsr.Text.Trim() $txtSrv.Text.Trim())
		if ([string]::IsNullOrWhiteSpace($txtDisp.Text) -or ($txtDisp.Text -eq $auto.LastAuto)) { $txtDisp.Text = $newDefault; $auto.LastAuto = $newDefault }
	})
	$txtUsr.add_TextChanged({
		$newDefault = (& $ComputeDefaultLabel $txtSub.Text $txtUsr.Text.Trim() $txtSrv.Text.Trim())
		if ([string]::IsNullOrWhiteSpace($txtDisp.Text) -or ($txtDisp.Text -eq $auto.LastAuto)) { $txtDisp.Text = $newDefault; $auto.LastAuto = $newDefault }
	})
	# If the user edits the Display name manually, stop auto-overwriting
	$txtDisp.add_TextChanged({ if ($txtDisp.Focused) { $auto.LastAuto = $null } })
	# Interval
	$lblInt=New-Object Windows.Forms.Label; $lblInt.Text= (T 'label.checkinterval'); $lblInt.Left=12; $lblInt.Top=220; $lblInt.AutoSize=$true
	$num=New-Object Windows.Forms.NumericUpDown; $num.Left=135; $num.Top=218; $num.Minimum=5; $num.Maximum=600; $num.Value=([int]$State.IntervalS); $num.Width=45
	# Language preference combo ("Auto" + available languages)
	$lblLang=New-Object Windows.Forms.Label; $lblLang.Text= (T 'label.language'); $lblLang.Left=190; $lblLang.Top=220; $lblLang.AutoSize=$true
	$cmbLang = New-Object Windows.Forms.ComboBox; $cmbLang.Left = 280; $cmbLang.Top = $lblLang.Top - 3; $cmbLang.Width = 60; $cmbLang.DropDownStyle='DropDownList'
	# Language import button (installed mode only)
	$btnLangImport = New-Object Windows.Forms.Button; $btnLangImport.Text = (T 'button.install_language'); $btnLangImport.Width = 200; $btnLangImport.Left = 360; $btnLangImport.Top = $lblLang.Top - 3
...
#				foreach ($f in $files) { $m = [regex]::Match($f.Name,'^NDTi18n\.([^.]+)\.json$'); if ($m.Success) { $langs += $m.Groups[1].Value } }
				foreach ($f in $files) {
					$m = [regex]::Match($f.Name, ('^{0}i18n\.([^.]+)\.json$' -f ([regex]::Escape($AppNameShort))))
		...
		$cmbLang.Items.Clear()
		[void]$cmbLang.Items.Add('Auto')
		foreach ($l in $langs) { [void]$cmbLang.Items.Add($l) }
		# Select current preference
		$pref = $State.LangPref
		if ([string]::IsNullOrWhiteSpace($pref)) {
			$cmbLang.SelectedItem = 'Auto'
		} else {
			if ($cmbLang.Items.Contains($pref)) {
				$cmbLang.SelectedItem = $pref
			} else {
				$cmbLang.SelectedItem = 'Auto'
			}
		}
	...
	$btnLangImport.Add_Click({
		try {
			...
			Show-ErrorT 'message.import_failed' @{ err = $_.Exception.Message }
		}
	})
	# Checkboxes for autostart and shortcuts
	$cbAuto = New-Object Windows.Forms.CheckBox; $cbAuto.Left=12; $cbAuto.Top=250; $cbAuto.Text = (T 'box.autostart'); $cbAuto.AutoSize=$true; $cbAuto.Checked=(Is-StartupRunKeyEnabled)
	$cbSM   = New-Object Windows.Forms.CheckBox; $cbSM.Left=200; $cbSM.Top=250; $cbSM.Text= (T 'box.shortcut_startmenu'); $cbSM.AutoSize=$true; $cbSM.Checked=(Shortcut-Exists 'StartMenu')
	$cbDesk = New-Object Windows.Forms.CheckBox; $cbDesk.Left=390; $cbDesk.Top=250; $cbDesk.Text= (T 'box.shortcut_desktop'); $cbDesk.AutoSize=$true; $cbDesk.Checked=(Shortcut-Exists 'Desktop')
	# Buttons for install, ex- import and uninstall 
	$btnInstall = New-Object Windows.Forms.Button; $btnInstall.Text = (T 'button.install2appdata'); $btnInstall.Left=12;  $btnInstall.Top=280; $btnInstall.Width=260
	$btnUninst  = New-Object Windows.Forms.Button; $btnUninst.Text = (T 'button.uninstall'); $btnUninst.Left=12; $btnUninst.Top=280; $btnUninst.Width=260
	$btnExportCfg = New-Object Windows.Forms.Button; $btnExportCfg.Text = (T 'button.export_config'); $btnExportCfg.Left=300;  $btnExportCfg.Top=280; $btnExportCfg.Width=260
	$btnImport  = New-Object Windows.Forms.Button; $btnImport.Text = (T 'button.import_config'); $btnImport.Left=12; $btnImport.Top=310; $btnImport.Width=260
	$btnExportP = New-Object Windows.Forms.Button; $btnExportP.Text = (T 'button.export2portable'); $btnExportP.Left=300; $btnExportP.Top=310; $btnExportP.Width=260
	# Buttons save and close
	$ok = New-Object Windows.Forms.Button; $ok.Text = (T 'button.save'); $ok.Left=350; $ok.Top=350; $ok.Width=100
	$ca = New-Object Windows.Forms.Button; $ca.Text = (T 'button.close'); $ca.Left=460; $ca.Top=350; $ca.Width=100
	if ($PortableMode) {
		$cbAuto.Enabled=$false; $cbSM.Enabled=$false; $cbDesk.Enabled=$false
		$btnExportP.Visible=$false; $btnUninst.Visible=$false; $btnInstall.Visible=$true
		$btnExportCfg.Visible=$false; $btnLangImport.Visible = $false
	} else {
		$btnInstall.Visible=$false; $btnExportP.Visible=$true; $btnUninst.Visible=$true
		$btnExportCfg.Visible=$true; $btnLangImport.Visible = $true
	}
	...
	$RebuildDriveList = {
		...
		$statuses.Clear()
		foreach ($d in $all) {
			$isCurrent = ($d -eq $current)
			$isUsed    = ($used -contains $d)
			if ($isCurrent) {
				# If dialog opened due to a conflict and we don't own the map, mark as USED
				if ($ConflictDrive -and ($d -ieq $ConflictDrive) -and (-not $script:mapMutexOwned)) {
					$statuses[$d] = 'used'
				} else {
					$statuses[$d] = 'current'
				}
			...
			[void]$cmb.Items.Add($d)
		}
		# Default selection: current if usable; otherwise first free
		if ($current -and $statuses.ContainsKey($current) -and ($statuses[$current] -in @('free','current'))) {
			$cmb.SelectedItem = $current
		} else {
			$firstFree = ($cmb.Items | Where-Object { $statuses[[string]$_] -eq 'free' } | Select-Object -First 1)
			if ($firstFree) { $cmb.SelectedItem = [string]$firstFree } else { $cmb.SelectedIndex = -1 }
		}
	}
	$cmb.DrawMode = 'OwnerDrawFixed'
	$cmb.ItemHeight = $cmb.Font.Height + 4
	$cmb.add_DrawItem({
		param($s,$e)
		if ($e.Index -lt 0) { return }
		$d = [string]$s.Items[$e.Index]
		$st = if ($statuses.ContainsKey($d)) { $statuses[$d] } else { 'free' }
		switch ($st) {
			'free'    { $bgColor = [System.Drawing.Color]::PaleGreen }
			'used'    { $bgColor = [System.Drawing.Color]::LightCoral }
			'current' { $bgColor = [System.Drawing.Color]::Khaki }
			default   { $bgColor = [System.Drawing.Color]::White }
		}
		if (($e.State -band [System.Windows.Forms.DrawItemState]::Selected) -ne 0) {
			...
			$fg = [System.Drawing.Brushes]::Black
		}
		$e.Graphics.DrawString($d,$e.Font,$fg,$e.Bounds.X+2,$e.Bounds.Y+2)
		$e.DrawFocusRectangle()
	})
	# Build once
	& $RebuildDriveList
	# Prevent selecting "used"; remember previous valid selection
	$prevSel = if ($cmb.SelectedItem) { [string]$cmb.SelectedItem } else { $null }
	$cmb.add_SelectionChangeCommitted({
		param($s,$e)
		$new = [string]$s.SelectedItem
		if ($statuses.ContainsKey($new) -and $statuses[$new] -eq 'used') {
			Show-WarnT 'message.drive_letter_used'
			if ($prevSel) {
				$s.SelectedItem = $prevSel
			} else {
				$firstFree = ($s.Items | Where-Object { $statuses[[string]$_] -eq 'free' } | Select-Object -First 1)
				if ($firstFree) { $s.SelectedItem = [string]$firstFree } else { $s.SelectedIndex = -1 }
			}
			return
		}
		$prevSel = $new
	...
	$ApplyLanguageNow = {
		...
		$setTip = { param($ctrl, $key); if ($ctrl -and ($ctrl -is [System.Windows.Forms.Control])) { $tip.SetToolTip($ctrl, (T $key)) } }
		# update dialog title
		& $setTxt $f (T 'title.settings_dialog' @{ app = $AppName })
		# update mode badge ("PORTABLE MODE"/"INSTALLED MODE")
		if ($lblMode) { if ($PortableMode) { & $setTxt $lblMode (T 'mode.portable'); $lblMode.ForeColor = [System.Drawing.Color]::ForestGreen } else { & $setTxt $lblMode (T 'mode.installed'); $lblMode.ForeColor = [System.Drawing.Color]::SteelBlue } }
		# update static labels / buttons in this window
		& $setTxt $lblSrv        (T 'label.server')
		& $setTxt $lblUsr        (T 'label.user')
		& $setTxt $lblPwd        (T 'label.app_password')
		& $setTxt $lblSub        (T 'label.subfolder')
		& $setTxt $lblDrv        (T 'label.drive_letter')
		& $setTxt $lblDisp       (T 'label.display_name')
		& $setTxt $lblInt        (T 'label.checkinterval')
		& $setTxt $lblLang       (T 'label.language')
		& $setTxt $btnLangImport (T 'button.install_language')
		& $setTxt $cbAuto        (T 'box.autostart')
		& $setTxt $cbSM          (T 'box.shortcut_startmenu')
		& $setTxt $cbDesk        (T 'box.shortcut_desktop')
		& $setTxt $btnInstall    (T 'button.install2appdata')
		& $setTxt $btnUninst     (T 'button.uninstall')
		& $setTxt $btnExportCfg  (T 'button.export_config')
		& $setTxt $btnImport     (T 'button.import_config')
		& $setTxt $btnExportP    (T 'button.export2portable')
		& $setTxt $ok            (T 'button.save')
		& $setTxt $ca            (T 'button.close')
		# force-refresh all tooltips to avoid stale cached strings
		$tip.RemoveAll()
		# apply i18n to static ToolTips
		& $setTip $txtSrv     'tip.enter_server'
		& $setTip $txtUsr     'tip.enter_user'
		& $setTip $btnPwdHelp 'title.app_password_help'
	...
	$cmbLang.add_SelectedIndexChanged({
		$sel = [string]$cmbLang.SelectedItem
		if ($sel -and $sel -ne 'Auto') {
			$State.LangPref = $sel
		} else {
			# "Auto" means: follow system
			$State.LangPref = ''
		}
		& $ApplyLanguageNow
	})
	...
	& $RefreshLangList
	# Button handlers
	$btnInstall.Add_Click({ Install-Self })
	$btnUninst.Add_Click({ Uninstall-Self })
	$btnExportCfg.Add_Click({ Export-InstalledConfig })
	$btnExportP.Add_Click({ Export-To-Portable })
	$btnImport.Add_Click({
		try {
			$dlg = New-Object System.Windows.Forms.OpenFileDialog
			...
			# Core fields
			foreach ($k in @('Server','User','SubPath','Drive','Label','IntervalS')) {
				if ($json.PSObject.Properties.Name -contains $k) { $State.$k = $json.$k }
			}
			...
			Save-Config
			# Reflect in UI
			$txtSrv.Text = $State.Server; $txtUsr.Text = $State.User
			# drive list refresh (keep used red & not selectable, pick first free)
			& $RebuildDriveList
			$prevSel = if ($cmb.SelectedItem) { [string]$cmb.SelectedItem } else { $null }
			# refresh language combo after import (LangPref might have changed)
			& $RefreshLangList
			# refresh map mutex to reflect new Drive/Server/User
			Ensure-MapMutex
	...
	$ok.Add_Click({
		...
		if ([string]::IsNullOrWhiteSpace($txtSrv.Text) -or [string]::IsNullOrWhiteSpace($txtUsr.Text) -or -not ($sel -match '^[A-Za-z]:$')) { Show-InfoT 'message.fill_server_user_drive'; return }
		...
		if ($PortableMode) {
			...
			if ((-not (Test-Path $SecretPath)) -and $txtPwd.Text -and ($txtPwd.Tag -ne 'info')) {
				$pp = Prompt-Passphrase (T 'title.passphrase_set' @{ app = $AppName }) -Confirm
				...
				try {
					...
					Show-ErrorT 'message.write_secret_failed' @{ err = $_.Exception.Message }
					return
				}
			}
		} else {
			# Installed: if user typed a new password, persist immediately (DPAPI)
			if ($txtPwd.Text -and ($txtPwd.Tag -ne 'info')) {
				try {
					Set-PlainPassword $txtPwd.Text
				} catch {
					Show-ErrorT 'message.store_password_failed' @{ err = $_.Exception.Message }
					return
				}
			}
		}
		# --- normalize subpath from UI (read-only textbox filled by picker) ---
		$valSub = $txtSub.Text.Trim()
		...
		if (-not [string]::IsNullOrWhiteSpace($valSub)) {
			...
			if ($PortableMode) {
				if (-not (Ensure-PortablePass -Interactive)) { return }
			}
			if (-not (Test-NcFolderExists $valSub)) {
				Show-WarnT 'message.folder_does_not_exist' @{ folder = $valSub }
				return
			}
		}
		# now update State (safe)
		$State.SubPath   = $valSub
		$State.Drive     = [string]$cmb.SelectedItem
		$State.Label     = $txtDisp.Text
		$State.IntervalS = [int]$num.Value
		# Language preference from combo ("Auto" means unset)
		$langSel = [string]$cmbLang.SelectedItem
		...
		if (-not $PortableMode) {
			try { Set-StartupRunKey ($cbAuto.Checked) } catch {}
			try { Ensure-Shortcut 'StartMenu' ($cbSM.Checked) } catch {}
			try { Ensure-Shortcut 'Desktop'   ($cbDesk.Checked) } catch {}
		}
		$driveChanged  = ($oldDrive  -and ($oldDrive -ne $State.Drive))
		$subChanged    = ($oldSubPath -ne $State.SubPath)
		$serverChanged = ($oldServer  -ne $State.Server)
		$userChanged   = ($oldUser    -ne $State.User)
		if ($driveChanged -or $subChanged -or $serverChanged -or $userChanged) {
			...
			if ($driveChanged -and (Test-ValidDrive $State.Drive)) {
				...
				$dGuardOk = $true
				try {
					Ensure-DriveGuard -Drive $State.Drive
				} catch {
					$dGuardOk = $false
				}
				if (-not $dGuardOk -or (-not $script:driveMutex)) {
					Show-WarnT 'message.drive_already_in_use_by_other_instance' @{ drive = $State.Drive; app = $AppName }
					return
				}
			}
			...
			try {
				Unmap-DriveIfOurs -drive $oldDrive -Force -RemoveProfile -Server $oldServer -User $oldUser -SubPath $oldSubPath
				try { Refresh-ExplorerDriveRemoval $oldDrive } catch {}
			} catch {}
			...
			# Nothing critical changed -> only ensure branding (idempotent)
			try { Ensure-BrandingTick } catch {}
		}
		Update-TrayImmediate -TryMap
	...
	$ca.Add_Click({ $f.DialogResult=[Windows.Forms.DialogResult]::Cancel; $f.Close() })
	$f.Controls.AddRange(@(
		$lblMode,
		$lblSrv,$picFav,$txtSrv,
		$lblUsr,$picAv,$txtUsr,
		$lblPwd,$btnPwdHelp,$txtPwd,$btnEncrypt,
		$lblSub,$txtSub,
		$lblDrv,$cmb,
		$lblDisp,$txtDisp,
		$lblInt,$num,$lblLang,$cmbLang,$btnLangImport,
		$cbAuto,$cbSM,$cbDesk,
		$btnInstall,$btnUninst,$btnExportCfg,$btnImport,$btnExportP,
		$ok,$ca
	))
	# Cleanup timers and images when dialog closes
	$f.add_FormClosed({
		try { if ($favTimer) { $favTimer.Stop(); $favTimer.Dispose() } } catch {}
		try { if ($avTimer) { $avTimer.Stop(); $avTimer.Dispose() } } catch {}
		try { if ($picFav -and $picFav.Image) { $picFav.Image.Dispose(); $picFav.Image = $null } } catch {}
		try { if ($picAv -and $picAv.Image) { $picAv.Image.Dispose(); $picAv.Image = $null } } catch {}
		try { if ($tip) { $tip.Dispose() } } catch {}
		# release singleton tracking
		$script:IsSettingsOpen = $false
		$script:SettingsForm = $null
	})
	# remember form reference so we can refocus it later
...
function Run-Watchdog {
	$created = $false
	try {
		$wdMutex = New-Object System.Threading.Mutex($true, $WDMutex, [ref]$created)
	...
	function Any-Main-Running {
		try {
			...
			$procs = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
				Where-Object {
					($_.CommandLine -match [regex]::Escape($ScriptFile)) -and
...
#			try { Load-Config } catch {}
			if (-not (Any-Main-Running)) { try { Unmap-DriveIfOurs -Force -RemoveProfile } catch {} }
			break
		}
		if ($PortableMode) {
			try {
...
function Get-InstalledScriptPath {
	# Returns absolute path to the installed NDT.ps1. Relies on $InstallDir being initialized earlier in the script.
	try {
		if ([string]::IsNullOrWhiteSpace($InstallDir)) { return $null }
		$path = Join-Path -Path $InstallDir -ChildPath 'NDT.ps1'
...
function Get-RunningInstalledMain {
	...
	try {
		...
		foreach ($p in $procs) {
			try {
				$dp = Get-Process -Id $p.ProcessId -ErrorAction Stop
				if ($dp.Id -ne $PID) { $out += $dp }
			} catch {}
		}
		return $out
	} catch { return @() }
...
function Stop-InstalledInstance {
	...
	foreach ($p in $procs) {
		try {
			if (-not $p.HasExited) {
				Write-Verbose ("Stopping PID {0} (soft)..." -f $p.Id)
	...
	foreach ($p in $procs) {
		try {
			$p.Refresh()
			if (-not $p.HasExited) { $still += $p }
		} catch {}
	}
	if ($still.Count -gt 0 -and $ForceOnTimeout) {
		foreach ($p in $still) {
			try {
				Write-Verbose ("Force-stopping PID {0}..." -f $p.Id)
	...
	foreach ($p in $procs) {
		try {
			$p.Refresh()
			if (-not $p.HasExited) { $rem += $p }
		} catch {}
	}
	if ($rem.Count -gt 0) {
		Write-Warning ("Some installed instances did not exit: {0}" -f ($rem.Id -join ', '))
		return $false
	}
	return $true
}
function Start-InstalledInstance {
	...
	try {
		...
		Write-Warning ("Start-InstalledInstance failed: {0}" -f $_.Exception.Message)
		return $false
	}
}
...
if ($script:NeedsSetup) {
	Write-Verbose 'First run detected -> opening Settings dialog immediately.'
	[void](Show-SettingsDialog)
	# Re-evaluate after dialog
	Refresh-NeedsSetup
}
...
if ($PortableMode -and (Test-Path $SecretPath) -and (-not $script:AskedPassphrase)) {
	while ($true) {
		...
		try { $script:PlainPassCache = Unprotect-PortableSecret $result $SecretPath; $script:AskedPassphrase = $true; break } catch {
			Show-WarnT 'message.passphrase_wrong_or_corrupt'
...
$miAbout.add_Click({
	...
	$ok = New-Object System.Windows.Forms.Button; $ok.Text = (T 'button.ok'); $ok.Width = 80; $ok.Left = $f.ClientSize.Width - $ok.Width - 16; $ok.Top = $f.ClientSize.Height - $ok.Height - 16; $ok.Anchor = 'Bottom,Right'; $ok.Add_Click({ $f.Close() })
	...
	if ($e.Button -eq [System.Windows.Forms.MouseButtons]::Left) {
		if ($script:NeedsSetup) {
			...
			$varsBalloon = @{
				...
				drive  = $State.Drive
				sub    = $subInfo
				status = Get-TrayStatusCore $script:tray.Text
			}
			$title = $AppName
			$text  = (T 'tray.balloon' $varsBalloon)
	...
	if ($PortableMode) {
		...
		if ($missingJson -or $missingSecret) {
			...
			if ($missingJson) {
				...
				$State.Drive  = ''
				$script:PlainPassCache = $null
				try { Rebuild-Icons } catch {}
			}
			$script:tray.Icon = $script:icoWarn
			$script:tray.Text = (T 'tray.secrets_missing' @{ app = $AppName })
			if (-not $script:MissingSecretsPrompted) {
				$script:MissingSecretsPrompted = $true
				$ans = Ask-YesNoWarnT 'prompt.secret_file_missing_quit' @{ app = $AppName }
				if ($ans -eq [System.Windows.Forms.DialogResult]::Yes) {
					if ($miExit) { $miExit.PerformClick() } else {
						try { if ($script:timer) { $script:timer.Stop() } } catch {}
						try { Unmap-Drive } catch {}
						try { if ($script:mainMutex) { $script:mainMutex.ReleaseMutex() | Out-Null; $script:mainMutex.Dispose() } } catch {}
